...
<!---PODRÍAMOS PONER UNA IMAGEN DE LA TABLA DEL TRAIN POR EJEMPLO, PARA VER QUE COLUMNAS TIENEN Y ASÍ CUANDO LAS QUITEMOS SE SEPA QUE SON------->
...
terms_frecuency_benefits_corpus_200 <- terms_frecuency_benefits_corpus[1:200]
terms_frecuency_benefits_corpus_200
```

Y visualizamos dichos términos gráficamente:

```{r}
graph_terms_frecuency_benefits_corpus <- as.matrix(terms_frecuency_benefits_corpus_200)
barplot(graph_terms_frecuency_benefits_corpus[1:200,],  xlab="Términos", ylab="Número de frecuencia",
...
terms_frecuency_benefits_corpus_200 <- as.matrix(terms_frecuency_benefits_corpus_200)
terms_frecuency_benefits_corpus_200
# Me quedo solo con los términos
terms_benefits_corpus_200 <- rownames(terms_frecuency_benefits_corpus_200)
terms_benefits_corpus_200
```
...
model = word2vec(train_file = "benefitsReview.txt", output_file = "benefitsReview.bin", binary=1)

dist_terms_benefits_corpus_200 = c()
# Obtengo la distancia de las 100 palabras con mayor frecuencia
for (i in 1:length(terms_benefits_corpus_200)){ # calculamos la distancia de la palabra a sus sinónimos
  dist_terms_benefits_corpus_200[i] = distance(file_name = "benefitsReview.bin", search_word = terms_benefits_corpus_200[i], num = 2)
}
```
...
terms_benefits_corpus_200[1]

# Vamos a sustituir "pain" por "swelling" y "nausea"
dist_terms_benefits_corpus_200[[1]]
```

Por último, ya solo nos queda hacer el reemplazamiento, para ello se usará la función *gsub(...)* sobre el corpus (benefits_corpus).

```{r, warning=FALSE}
# Para la columna benefitsReview del conjunto train
as.character(dist_terms_benefits_corpus_200[[2]][3])

for (i in 1:length(terms_benefits_corpus_200)) # iteramos sobre los terminos
  for (j in 1:2) # iteramos sobre los sinónimos, en este caso solo tenemos 2
    # Con "tm_map" NO FUNCIONA
    #x1 <- tm_map(benefits_corpus, content_transformer(gsub), pattern = as.character(dist_terms_benefits_corpus_200[[i]][j]), 
    #             replacement = as.character(terms_benefits_corpus_200[i]))
    x1 <- gsub(as.character(dist_terms_benefits_corpus_200[[i]][j]), as.character(terms_benefits_corpus_200[i]),
               benefits_corpus)

write.table(benefits_corpus$content, "sinSinonimos.txt")
write.table(x1$content, "conSinonimos.txt")
...
for (i in 1:length(terms_benefits_corpus_200)) # iteramos sobre los terminos
  for (j in 1:2) # iteramos sobre los sinónimos, en este caso solo tenemos 2
    hola <- gsub(subdf[i,j], terminos[i], hola, fixed = TRUE)
    #gsub(subdf[i,j], terminos[i], benefits_corpus)
    #x1 <- tm_map(benefits_corpus, content_transformer(gsub), pattern = as.character(subdf[i,j]), replacement = as.character(terminos[i]))
hola
sin <- write.table(benefits_corpus$content, "sinSinonimos.txt")
write.table(hola, "conSinonimos9.txt")
benefits_corpus$content
x1$content
```
...
con su valor correspondiente de distancia, nos quedamos con las que superan un umbral de 0.7:

```{r}
dist_terms_benefits_corpus_100[[1]]$dist
(dim(dist_terms_benefits_corpus_100)[1])[2]
as.vector(dist_terms_benefits_corpus_100)
matrix_dist_terms_benefits_corpus_100 <- as.matrix(dist_terms_benefits_corpus_100)
matrix_dist_terms_benefits_corpus_100[100][2]

temp = c()
if (as.vector(dist_terms_benefits_corpus_100[[1]]$dist) > 0.70) {
  temp[1] = dist_terms_benefits_corpus_100[1]
}

temp = c()
for (i in 1:length(terms_benefits_corpus_100)){
  aux <- as.matrix(dist_terms_benefits_corpus_100[[i]])
  for (j in 1:dim(dist_terms_benefits_corpus_100)[1]){
    if (as.vector(dist_terms_benefits_corpus_100[[j]]$dist) > 0.70) 
      temp[j] = aux[j]
  }
}
...
a <- as.matrix(dist_terms_benefits_corpus_100)
nueva = c()

for (i in 1:dim(dist_terms_benefits_corpus_100)[1]){
  if (as.vector(dist_terms_benefits_corpus_100$dist[i]) > 0.70 ) nueva[i] =a[i]
}
nueva
...
```{r}
ana = word_analogy(file_name = "vec.bin", search_words = "weeks times year",num = 10)
ana
```
