...
\hypersetup{unicode=true,
            pdfauthor={Alejandro Campoy Nieves; Gema Correa FernÃ¡ndez; Luis Gallego Quero; Jonathan MartÃ­n Valera; Andrea Morales GarzÃ³n},
            pdfborder={0 0 0},
            breaklinks=true}
...
\setlength{\droptitle}{-2em}
  ...
  \posttitle{}
    \author{Alejandro Campoy Nieves \\ Gema Correa FernÃ¡ndez \\ Luis Gallego Quero \\ Jonathan MartÃ­n Valera \\ Andrea Morales GarzÃ³n}
...
\begin{center} \Large \textbf{\textsc{PrÃ¡cticas de la asignatura}} \end{center}
...
\textbf{\large En colaboraciÃ³n con:} \vspace{0.2cm}
...
\hspace{8.5cm}{Gema Correa FernÃ¡ndez:  \href{mailto:gecorrea@correo.ugr.es}{\textcolor{blue}{\underline{gecorrea@correo.ugr.es}}} }
...
\hspace{8.5cm}{Andrea Morales GarzÃ³n:  \href{mailto:andreamgmg@correo.ugr.es}{\textcolor{blue}{\underline{andreamgmg@correo.ugr.es}}} }
...
\lhead{Proyecto: TÃ©cnicas aplicadas para anÃ¡lisis inteligente de datos}
\rhead{\thepage} \setcounter{page}{1}

\section{1. Comprender el problema a
resolver}\label{comprender-el-problema-a-resolver}

Para la realizaciÃ³n y aplicaciÃ³n de las tÃ©cnicas explicadas a lo
largo del curso, hemos seleccionado un \emph{dataset} proporcionado por
...
organizada por relevancia para medicamentos especÃ­ficos. El conjunto de
datos proporciona revisiones de pacientes sobre medicamentos
especÃ­ficos junto con las condiciones relacionadas. AdemÃ¡s, las
revisiones se agrupan en informes sobre tres aspectos: beneficios,
efectos secundarios y comentarios generales. De igual modo, las
calificaciones estÃ¡n disponibles con respecto a la satisfacciÃ³n
general, asÃ­ como una calificaciÃ³n de efectos secundarios y de
eficacia de 5 pasos. Los datos se obtuvieron rastreando los sitios de
revisiÃ³n farmacÃ©utica en lÃ­nea.

El objetivo principal del estudio es:

\begin{itemize}
\item
  Realizar un anÃ¡lisis de sentimientos en relaciÃ³n con la experiencia
  en el uso de dichos medicamentos, como por ejemplo la efectividad,
...
En este proyecto nos centraremos en el \textbf{anÃ¡lisis y experiencia
de los usuarios} en el uso de los distintos medicamentos.

Las caracterÃ­sticas de este conjunto de datos vienen descritas en la
siguiente tabla:
...
\item
  \textbf{condition} (categorical): nombre de la condiciÃ³n
\item
...
\item
  \textbf{rating} (numerical): clasificaciÃ³n de paciente de 10
  estrellas
\item
  \textbf{sideEffects} (categorical): clasificaciÃ³n de 5 pasos de
  efectos secundarios
\item
  \textbf{effectiveness} (categorical): clasificaciÃ³n de efectividad de
  5 pasos
\end{enumerate}
...
A continuaciÃ³n, leemos nuestro dataset train y test:
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{summary}\NormalTok{(datos_train) }\CommentTok{# informaciÃ³n sobre los datos}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{summary}\NormalTok{(datos_test) }\CommentTok{# informaciÃ³n sobre los datos}
\end{Highlighting}
\end{Shaded}
...
\subsection{2.2. Falta de datos, categorizaciÃ³n, normalizaciÃ³n,
reducciÃ³n de
dimensionalidad.}\label{falta-de-datos-categorizacian-normalizacian-reduccian-de-dimensionalidad.}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Procesar datos}

\CommentTok{# 1. Eliminamos la columna del ID. Esa columna es la número 1, por tanto la quitamos directamente del data_frame}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# 2. Primero tenemos que usar la librería que procese los datos de tipo texto en R. La más conocida se llama tm. Si no la tenemos instalada en R, tnemos que hacerlo previamente. Para eso hacemos:}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# 3. Los datos que vamos a leer se cargan haciendo un vector de mensajitos. Para eso, nos creamos un vector de documentos a partir de un vector de string. Podemos usar la función VectorSource para hacer esta conversión.}
...
\NormalTok{beneficts_review_data =}\StringTok{ }\KeywordTok{as.vector}\NormalTok{(datos_test}\OperatorTok{$}\NormalTok{benefitsReview)}

\CommentTok{# Lo convertimos en la estructura de documento, y lo guardamos ya en el corpus que lo vamos a utilizar.}
\NormalTok{corpus =}\StringTok{ }\NormalTok{(}\KeywordTok{VectorSource}\NormalTok{(beneficts_review_data))}
...
\CommentTok{# Podemos ver que funciona accediendo a uno cualquiera. Si nos fijamos en el contenido, vemos que tiene signos de puntuación y exclamación.}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# 4. Una vez que tenemos el corpus creado, continuamos con el procesamiento. En data mining no tiene sentido contemplar los signos de puntuación, por lo que los quitamos.}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Si volvemos a mostrar la opinión cuarta, vemos como todos los signos han desaparecido. De hecho, podemos inspeccionar el corpus, y se ve como todos los signos de puntuación, exclamación y derivados ya no están.}
\KeywordTok{inspect}\NormalTok{(corpus[}\DecValTok{4}\NormalTok{])}
\end{Highlighting}
\end{Shaded}
