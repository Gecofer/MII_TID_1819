...
En el siguiente capítulo nos enfrentamos al desarrollo de la clasificación no supervisada de patrones en grupos, mas conocido como clustering. Dichos grupos se establecen como particiones en las que las observaciones pertenencientes son similares entre ellas y distintas a las observaciones del resto de particiones.

## Lectura y adecuación de los datos

Como proceso anterior al punto en el que nos encontramos, se realizó todo el preprocesado de los datos. Ahora, para su uso en las técnicas de clustering, debemos de tomar las columnas de datos que más nos interesan para su análisis. 

Para ello, primero establecemos una semilla común, para así poder obtener siempre los mismos datos, y posteriormente leemos nuestros dataset tanto para el conjunto de train como de test con dicho preprocesado realizado. Además, nos quedaremos con las columnas de ambos dataset que más nos interesan, siendo estas el rating, sideEffectsInverse y effectivenessNumber.

##### NOTA: Explicar porqué tomamos esas columnas
...
Esta selección inicial de columnas no es suficiente para poder empezar a aplicar las técnicas, antes debemos de arreglar el conjunto de datos que hemos tomado.......
...
La idea básica del presente método es que agrupa las observaciones en K clusters distintos, siendo el valor de K preestablecido mediante análisis previos. Entonces, K-medias encuentra los K mejores clusters, siendo estos aquellos cuya varianza interna sea lo más pequeña posible.

En este caso, vamos a establecer un valor de K igual a 2, para así poder predecir el número de aciertos más adelante.
...
head(res,10)
var = rownames(as.matrix(res_clustering$cluster))
length(res[,1])
mat = cbind(var,res[,1])

# df con la etiqueta a 1
xy= x[,1]==1
zyy = data_train_procesado[xy,]
mean(zyy$effectivenessNumber)

xy2= x[,1]==2
zyy2 = data_train_procesado[-xy,]
mean(zyy2$effectivenessNumber)

m
```

Como comprobación del buen ajuste de la división de clusters, en el siguiente gráfico podemos ver la similitud y cohesión entre estos.
...
Como hemos comentado anteriormente, vamos a intentar predecir mediante el clustering que nos ha generado train el cluster al que pertenecería cada valor de test y comprobaremos el acierto de dicha predicción.
