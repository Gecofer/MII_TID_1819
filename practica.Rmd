...
tidy_document1 <- data_frame(line1 = 1:3107, text = datos_train$benefitsReview)
document_bigrama1 <- tidy_document1 %>%
  unnest_tokens(benefits, text, token = "ngrams", n = 1) 
tidy_document2 <- data_frame(line2 = 1:3107, text = datos_train$sideEffectsReview)
document_bigrama2 <- tidy_document2 %>%
  unnest_tokens(effects, text, token = "ngrams", n = 1) 


r <- merge(document_bigrama1, document_bigrama2)

tweets_spread <- tidy_document2 %>% group_by(line, benefits) %>% count(benefits) %>%
                 spread(key = line, value = n, fill = NA, drop = TRUE)

cor.test(~ document_bigrama1 + document_bigrama1, method = "pearson", data = tweets_spread)
...
res <- cbindPad(document_bigrama1,document_bigrama2)
...
dim(document_bigrama1)

# de cada variable en una columna. 
tweets_spread <- tidy_document2 %>% group_by(line, effects) %>% count(effects) %>%
                 spread(key = line, value = n, fill = NA, drop = TRUE)


res <- cbind(document_bigrama1,document_bigrama2)
...
tweets_spread <- tweets_tidy %>% group_by(word, token) %>% count(token) %>%
...
pairs(cbind(datos_train$sideEffectsNumber, datos_train$effectivenessNumber), lower.panel=panel.smooth,upper.panel=panel.cor)
```


```{r}
res <- cor.test(datos_train$sideEffectsNumber, datos_train$effectivenessNumber, 
                    method = "pearson")
res
```
...
```{r}
library(corrplot)
corrplot(cbind(datos_train$sideEffectsNumber, datos_train$effectivenessNumber), type = "upper", order = "hclust", 
...
cor.test(~ datos_train$benefitsReview + datos_train$benefitsReview, method = "pearson", data = datos_train)
```
...
library(corrplot)
corrplot(datos_train, type = "benefitsReview", order = "sideEffectsReview", 
         tl.col = "black", tl.srt = 45)
```


```{r}
corpus <- process_texts(datos_train$benefitsReview, stem= TRUE)
dtm <- DocumentTermMatrix(corpus) 
findAssocs (dtm, "benefit", corlimit = 0.3)
```
...
cor.test(~ mayoredlee, method = "pearson", data = tweets_spread)
```
```{r}
library("ggpubr")
gscatter(datos_train, x = "benefitsReview", y = "sideEffectsReview", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.method = "pearson",
          xlab = "Miles/(US) gallon", ylab = "Weight (1000 lbs)")

ggqqplot(datos_train$benefitsReview, ylab = "benefitsReview")
res <- cor.test(as.vector(datos_train$benefitsReview),as.vector(datos_train$sideEffectsReview), 
                    method = "pearson")
```
```{r}
library(gridExtra)
...
tweets_spread <- tweets_tidy %>% group_by(word, token) %>% count(token) %>%
                 spread(key = word, value = n, fill = NA, drop = TRUE)
...
map_df(list(datos_train$benefitsReview, NA, datos_train$sideEffectsReview), try_filter)


tweets <- bind_rows(datos_train$benefitsReview, datos_train$sideEffectsReview)
...
```
```{r}
# Juntamos las columnas datos_train$benefitsReview y datos_train$sideEffectsReview
```
...
vector <- c()
for (i in 1:5){ # calculamos la distancia de la palabra a sus sinónimos
...
#doc_corpus <- benefits_train_corpus
...
```{r}
ggplot() + 
 geom_bar(data=z,aes(x=z$terms1_5, y=z$v), stat='identity', position='dodge') 
```
