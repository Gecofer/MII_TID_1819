...
```{r}
benefits_cambiados = for (j in 1:5){ iris2[,j]=ordered(cut(iris[,j],breaks=quantile(iris[,j])),labels=c("Extremely Severe side Effects","Mild Side Effects","Moderate Side Effects","No Side Effects", "Severe Side Effects"))}


train_trans=as(benefits_review_train[3],"transactions")
summary(train_trans)
```
